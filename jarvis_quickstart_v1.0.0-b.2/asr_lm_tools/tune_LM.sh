...
if [ "$#" -lt 3 ]; then
  echo "Usage: $0 <jarvis_config_file> <LM_tuning_config_file> <decoder_type (cpu|gpu)"
  echo " "  
  echo "   where <jarvis_config_file> is absolute path to Jarvis config file (config.sh)"  
...
jarvis_config_file=${1}
lm_tuning_config_file=${2}
decoder_type=${3}

# Creating new config file with only ASR service enabled
new_jarvis_config_file=$(mktemp /tmp/jarvis_config.XXXXXX)
echo "New config file: $new_jarvis_config_file"
cp $jarvis_config_file $new_jarvis_config_file

#Disable all services except ASR
echo "service_enabled_asr=true" >> $new_jarvis_config_file
echo "service_enabled_nlp=false" >> $new_jarvis_config_file
echo "service_enabled_tts=false" >> $new_jarvis_config_file

source $new_jarvis_config_file
...
#Launch Jarvis
jarvis_start_cmd="$script_path/../jarvis_start.sh ${new_jarvis_config_file}"

client_cmd="$update_manifest_cmd; /usr/local/bin/jarvis_asr_client --word_time_offsets=false --jarvis_uri=localhost:50051 --automatic_punctuation=false --audio_file=/wav/manifest.tmp.json --output_filename=/wav/output.json; python3 /work/utils/calc_wer.py -test /wav/output.json -ref /wav/manifest.tmp.json"
jarvis_cmd="docker run --init --rm $mnt_args --net host --name ${jarvis_daemon_client} $image_client /bin/bash -c \"$client_cmd\""

echo "Jarvis command:"
echo $jarvis_cmd

now=`date +%Y-%m-%d_%H:%M:%S`
tuning_results_filename="jarvis_lm_tuning_${now}.csv"
...
if [[ $decoder_type == "cpu" ]]; then
  ...
  do
        echo "Terminating Triton and Jarvis server"
        docker kill $jarvis_daemon_speech &> /dev/null
  
        # Modify the Triton config file
        docker run --init --rm -v $script_path:/tmp/ -v $jarvis_model_loc:/data/ ubuntu:18.04 /bin/bash -c "/tmp/modify_config_param.sh ${triton_config_file} beam_search_width ${beam_width}; /tmp/modify_config_param.sh ${triton_config_file} language_model_alpha ${alpha}; /tmp/modify_config_param.sh ${triton_config_file} language_model_beta $beta"
  
        echo "Launching Triton and Jarvis server"
        eval $jarvis_start_cmd 
  
        echo "Running ASR with beam_width $beam_width, alpha $alpha, beta $beta"
        echo "jarvis_cmd: $jarvis_cmd"
        docker kill jarvis-asr-client &> /dev/null; eval $jarvis_cmd &> output_tmp 
