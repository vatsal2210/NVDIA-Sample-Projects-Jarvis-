...
block_until_server_alive() {
    for i in {1..30}
    do
    docker exec $1 /bin/grpc_health_probe -addr=:$jarvis_speech_api_port 2> /dev/null
    rc=$?
    if [ $rc -ne 0 ]; then
      echo "Waiting for Jarvis server to load all models...retrying in 10 seconds"
      sleep 10
    else
      echo "Jarvis server is ready..."
      exit 0
    fi
    done
    echo "Health ready check failed."
    echo "Check Jarvis logs with: docker logs $jarvis_daemon_speech"
    exit 1
}
...
if [ "$service_enabled_asr" = true ] || [ "$service_enabled_nlp" = true ] || [ "$service_enabled_tts" = true ]; then
    ...
    ld_preload+=$(docker run --init -it --rm -v $jarvis_model_loc:/data --entrypoint "/bin/bash" $image_speech_api -c "find /data/plugins/*.so -type f ! -size 0 2>/dev/null" | sed 's/\r//g' | paste -sd ':' -)
fi

# speech server is required
# check if it's already running first...
if [ $(docker ps -q -f "name=^/$jarvis_daemon_speech$" | wc -l) -eq 0 ]; then
    echo "Starting Jarvis Speech Services. This may take several minutes depending on the number of models deployed."
    docker rm $jarvis_daemon_speech &> /dev/null
    docker run -d \
        --init \
        --gpus '"'$gpus_to_use'"' \
        -p 8000 -p 8001 -p 8002 -p $jarvis_speech_api_port:$jarvis_speech_api_port \
        -e "LD_PRELOAD=$ld_preload" \
        -v $jarvis_model_loc:/data \
        --ulimit memlock=-1 --ulimit stack=67108864 \
        --name $jarvis_daemon_speech $image_speech_api \
    start-jarvis --jarvis-uri=0.0.0.0:$jarvis_speech_api_port \
    --asr_service=$service_enabled_asr --tts_service=$service_enabled_tts --nlp_service=$service_enabled_nlp &> /dev/null
else
    echo "Jarvis Speech already running. Skipping..."
fi
block_until_server_alive $jarvis_daemon_speech
