...
if [ "$service_enabled_asr" = true ] || [ "$service_enabled_nlp" = true ] || [ "$service_enabled_tts" = true ]; then
    echo "> Pulling Jarvis Speech Server images."
...
if [ "$use_existing_jmirs" = false ]; then
    echo
    echo "Downloading models (JMIRs) from NGC..."
    echo "Note: this may take some time, depending on the speed of your Internet connection."
    echo "To skip this process and use existing JMIRs set the location and corresponding flag in config.sh."
...
# convert all jmirs
echo
echo "Converting JMIRs at $jarvis_model_loc/jmir to Jarvis Model repository."

set -x
docker run --init -it --rm --gpus '"'$gpus_to_use'"' \
  -v $jarvis_model_loc:/data \
  -e "MODEL_DEPLOY_KEY=${MODEL_DEPLOY_KEY}" \
  $image_init_speech deploy_all_models /data/jmir /data/models
